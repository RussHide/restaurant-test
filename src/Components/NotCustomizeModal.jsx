...
const NotCustomizeModal = ({ open, setOpen, product, hasDiscount = false }) => {
        ...
        <Modal width={1000} footer={[
            <div key={1} className="flex justify-around items-center p-3 gap-x-4">
                <div className='bg-gray-100 flex justify-center items-center gap-7 px-4 rounded-lg py-2 w-1/3'>
                    ...
                    <span className='font-semibold text-lg'>{productAmout}</span>
                    <FiPlus onClick={() => setProductAmout(productAmout + 1)} className='cursor-pointer' size={22} />
                </div>
                <button onClick={btnSubmit} className="bg-[#F6782E] hover:bg-[#E6681E] text-white px-8 md:px-4 py-2.5 flex justify-between rounded-xl font-semibold w-2/3"><span>Agregar al carrito</span><span>MX ${hasDiscount ? (product.precio / 2).toFixed(2) : product.precio}</span></button>
            </div>
...
export default NotCustomizeModal